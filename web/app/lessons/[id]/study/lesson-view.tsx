...
import { ArrowLeft, ArrowRight, Star, Clock, Hash } from "lucide-react";
...
export function LessonView({ data, next }: { data: LessonData; next: Lesson | null }) {
  ...
  // Switch to first available tab on mount
  useEffect(() => {
  ...
  const handleComplete = async () => {
    const r = await completeLessonAction({ lessonId: lesson.id });
    if (r.ok) router.push(`/lessons/${lesson.id}/result?attempt=${r.attemptId}`);
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-stone-50">
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-6">
        {/* Left panel */}
        ...
        <div className="lg:sticky lg:top-20 lg:self-start space-y-3 min-w-0">
              ...
              {hasQuiz && (
                <TabsContent value="quiz">
                  <QuizTab
                    ...
                    onDone={() => setCompleted((c) => ({ ...c, quiz: true }))}
                  />
                </TabsContent>
              )}
          ...
          <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-3 flex items-center justify-between">
            ...
            >
              {allDone ? "Hoàn thành" : "Cần làm xong bài tập"} <ArrowRight className="size-4" />
            </button>
          </div>
        </div>
      </div>
