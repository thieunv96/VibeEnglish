...
### Database
Prisma 6 with MariaDB 11. Schema lives in `prisma/schema.prisma`. Key models: `User`, `Lesson`, `LessonProgress`, `LessonRating`, `Exercise`, `ExerciseAttempt`, `MockTestAttempt`, `VocabItem`, `UserActivity`. `UserActivity` stores minute-level timestamps used by `computeStreakDays()` for streak logic. `MockTestAttempt` stores parent records for listening mock tests; each parent has N child `ExerciseAttempt` rows (written atomically). After schema changes always run `npm run db:generate`.
...
| `shuffle.ts` | Fisher-Yates shuffler — extracted utility used by test-prep and sample test sampling |
| `test-prep-constants.ts` | Exam slug enum, `attemptType` enum, `MOCK_TEST_QUESTION_COUNT` constant |
| `test-prep-bands.ts` | `estimateBand(exam, correct, total): BandResult` — per-exam band estimation; thresholds are placeholders (NQ-1) |
| `test-prep-content.ts` | `sampleListeningQuestions(exam, count)` — server-side sampler for listening-only mock questions |
| `test-prep-progress.ts` | `getExamProgress(userId, exam)` — aggregates progress from `MockTestAttempt` records |
| `test-prep-admin-analytics.ts` | `getTestPrepAnalytics()` — per-exam analytics for admin dashboard; consumed by `/api/admin/test-prep/analytics` |
