...
// Generated once; sorted by English name.
...
function flag(cc: string): string {
  // Flag emoji = "Regional Indicator Symbol Letter" of each letter (0x1F1E6 + (A-A))
...
const RAW: Array<[string, string]> = [
  ...
  ["CZ", "Czechia"], ["CD", "Democratic Republic of the Congo"], ["DK", "Denmark"],
...
export const COUNTRIES: ReadonlyArray<Country> = RAW
  .map(([code, name]) => ({ code, name, flag: flag(code) }))
  .sort((a, b) => a.name.localeCompare(b.name));

export const COUNTRY_CODES: ReadonlyArray<string> = COUNTRIES.map((c) => c.code);
